...
if (cartContainer) {
  ...
  const fetchAllCartItems = async () => {
    const data = await fetch(
      `${slashes}/src/pages/ajax/cart.php?read=${userId}&continuation_of_cart_process=${continuation_of_cart_process}`,
      {
        method: 'GET',
      }
    )
  ...
  const fechtTotalPrice = async () => {
    const data = await fetch(
      `${slashes}src/pages/ajax/cart.php?readTotalPrice=${userId}`,
      {
        method: 'GET',
      }
    )
  ...
  if (addForms) {
    ...
    addForms.forEach((addForm) => {
      addForm.addEventListener('submit', async (e) => {
        ...
        let data = await fetch(`${slashes}src/pages/ajax/cart.php`, {
          method: 'POST',
          body: formData,
        })
